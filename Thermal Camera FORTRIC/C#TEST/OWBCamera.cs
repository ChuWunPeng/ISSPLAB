...
namespace TEST
{
    ...
    public class OWBCamera
    {
        ...
        public OWBCamera()
        {
            ...
            RecordGrabberCallback = GrabberRecord;
        ...
        private OWBVideoSDK Video { get; set; }
            ...
            switch (VideoMode)
            {
                ...
                case StreamSDK.URL_STREAM_VIDEO_SUB:
                    ...
                    try
                    {
                        ...
                        width = streamVideoSub.Width;
                        height = streamVideoSub.Height;
                    }
                    catch
                    {
                        return false;
        ...
        public bool StartRecordStream(string fileName)
        {
            Video = new OWBVideoSDK(fileName);
        ...
        private bool StartRecordStream()
        {
            int max_packet_size = 0;
            double fps = 0;
            ...
            if (StreamSDK.streamsdk_start_stream(HRecordStream, StreamSDK.URL_STREAM_VIDEO_PRI, max_packet_size, null, IntPtr.Zero) != StreamSDK.STREAMSDK_EC_OK)
            {
                return false;
            }
            ...
            dp.dec_h = Height;
            dp.pix_type = (int)StreamSDK.streamsdk_enum_pix_type.STREAMSDK_PIX_BGR;
            StreamSDK.streamsdk_create_h264_decoder(HRecordStream, ref dp, ref dec);
            RecordDecoder = dec;
            StreamSDK.streamsdk_start_h264_decoder(RecordDecoder, RecordGrabberCallback, IntPtr.Zero);
            Video.CreateVideoStream(fps, Width, Height);
            return true;
        }

        private bool CreateRecordStream()
        {
            StreamSDK.streamsdk_set_thread_pool_size(4);
            IntPtr hStream = IntPtr.Zero;
            if (StreamSDK.streamsdk_create_stream(IP, (ushort)(PORT + 1), ref hStream) != StreamSDK.STREAMSDK_EC_OK)
            ...
            HRecordStream = hStream;
            return true;
        }

        public void StopRecordStream()
        {
            if (RecordDecoder != IntPtr.Zero)
            {
                StreamSDK.streamsdk_stop_h264_decoder(RecordDecoder);
                StreamSDK.streamsdk_destroy_h264_decoder(RecordDecoder);
                RecordDecoder = IntPtr.Zero;
            }
            if (HRecordStream != IntPtr.Zero)
            {
                StreamSDK.streamsdk_stop_stream(HRecordStream);
                StreamSDK.streamsdk_destroy_stream(HRecordStream);
                HRecordStream = IntPtr.Zero;
            }
            ...
            lock (CameraLock)
            {
                ...
                if (OWBTypes.CheckStatus(RestSDK.Get(IP, (ushort)PORT, URL_SENSOR_CURRENT_LUT_INDEX, ref buf, ref length)))
                {
                    ...
                    for (int i = 0; i < sensorLUTs.Length - 1 && sensorLUTs[i].R < sensorLUTs[i + 1].R; i++)
                    {
                        ...
                        {
                            lut[j] = fromTemp + (toTemp - fromTemp) * (j - tmpFrom) / (tmpTo - tmpFrom) + ZERO_T;
                        }
                    }
                    ...
                    From = from;
                    To = to;
                    OWBTypes.InstrumentJconfig instrumentJconfig = GetInstrumentJconfig();
                    Radiation.CalculateFromFactoryLUT_SH(instrumentJconfig.Emission, instrumentJconfig.Reflection_t + ZERO_T,
                        instrumentJconfig.Atmosphere_t + ZERO_T, instrumentJconfig.Distance, instrumentJconfig.RH,
                        instrumentJconfig.Lens_transmission, instrumentJconfig.Lens_t + ZERO_T, lut, from, to);
                    LUT = lut;
                }
            }
        }
                ...
                if (RawValues != null && RawValues.GetLength(0) == Width && RawValues.GetLength(1) == Height)
                {
                    ...
                    for (int i = 0; i < Width; i++)
                    {
                        for (int j = 0; j < Height; j++)
                        {
                            ...
                            {
                                values[i, j] = LUT[From] - ZERO_T;
                            }
                            else if (RawValues[i, j] > To)
                            {
                                values[i, j] = LUT[To] - ZERO_T;
                            }
                            else
                            {
                                values[i, j] = LUT[RawValues[i, j]] - ZERO_T;
                            }
                        }
                    }
                    return true;
        ...
        private void GrabberH264(int error, ref StreamSDK.streamsdk_st_image image, IntPtr user_data)
        {
            ...
            try
            {
                using (Bitmap bitmap = new Bitmap(image.img_w, image.img_h, PixelFormat.Format24bppRgb))
                {
                    BitmapData bmpData = bitmap.LockBits(new Rectangle(0, 0, image.img_w, image.img_h), ImageLockMode.WriteOnly, PixelFormat.Format24bppRgb);
                    int stride = bmpData.Stride;
                    int offset = stride - image.img_w * 3;
        ...
        private void GrabberRecord(int error, ref StreamSDK.streamsdk_st_image image, IntPtr user_data)
        {
            if (error != StreamSDK.STREAMSDK_EC_OK)
            {
                return;
            ...
            try
            {
                using (Bitmap bitmap = new Bitmap(Width, Height, PixelFormat.Format24bppRgb))
                {
                    ...
                    bitmap.UnlockBits(bmpData);
                    Image coverImage = bitmap.Clone(new Rectangle(0, 0, bitmap.Width, bitmap.Height), bitmap.PixelFormat);
                    RecordVideo(coverImage);
                }
            }
            catch
            {
        ...
        private void RecordVideo(Image image)
        {
