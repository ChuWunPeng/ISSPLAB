...
#include "../../sdk/restc.h"
#pragma comment(lib,"../../sdk/restc.lib")
...
public:
	...
	/* HTTP GET 请求 */
	void restGet(const std::string & url_path,Json::Value & reply);
	void restGetBin(const std::string & url_path,char * buf);
	/* HTTP PUT 请求, 设备返回信息是可选的 */
	void restPut(const std::string & url_path,const Json::Value & request,Json::Value *preply);
	/* HTTP POST 请求，设备返回信息是可选的 */
	void restPost(const std::string & url_path,const Json::Value & request,Json::Value *preply);
	/* HTTP DELETE 请求 */
	void restDel(const std::string & url_path);

private:
	/** POST 和 PUT 共用 **/
	void _restPut(const std::string & url_path,const Json::Value & request,Json::Value *preply
