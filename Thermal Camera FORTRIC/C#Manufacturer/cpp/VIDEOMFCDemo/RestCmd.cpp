...
void
RestCmd::restGet(const std::string & url_path,Json::Value & reply)
{
	std::size_t length = 0;

	/** RestHeaders 为空时表示没有额外的HTTP头 **/
	int statusCode = restc_get(_rest_conn,url_path.c_str(),NULL,_rest_buf,RESTCMD_BUFSZ,&length);
...
void
RestCmd::restDel(const std::string & url_path)
{
	std::size_t length = 0;

	/** RestHeaders 为空时表示没有额外的HTTP头 **/
	int statusCode = restc_del(_rest_conn,url_path.c_str(),NULL,_rest_buf,RESTCMD_BUFSZ,&length);
	if(statusCode != HTTP_OK)
...
void
RestCmd::restPut(const std::string & url_path,const Json::Value & request, Json::Value * preply)
{
	_restPut(url_path,request,preply,restc_put);
}

void
RestCmd::restPost(const std::string & url_path,const Json::Value & request, Json::Value * preply)
{
	_restPut(url_path,request,preply,restc_post);
}

void
RestCmd::_restPut(const std::string & url_path,const Json::Value & request, Json::Value * preply
			,int (* put_func )(RestConn ,const char * ,RestHeaders ,char * ,size_t ,size_t * ))
{
	...
	int statusCode = put_func(_rest_conn,url_path.c_str(),NULL,_rest_buf,RESTCMD_BUFSZ,&length);
